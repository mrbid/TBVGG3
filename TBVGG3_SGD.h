...
--------------------------------------------------
    ...
    Configuration;
        ...
        I do feel that SGD fits real-time training better than NAG.
        Because the learning rate is a define you could turn it into
        a random step size such as;
        `#define LEARNING_RATE (((float)(rand()%10))/1000.f)`
        which would give you a random learning rate between 0.01 and 0.001
        of 10 evenly spaced increments.
    ...
    Network size:
        (3x28x28)+(32x3x9)+(64x32x9)+(128x64x9)+(32x28x28)+
        (32x14x14)+(64x14x14)+(64x7x7)+(128x7x7)+(32x28x28)+
        (64x14x14)+(128x7x7)+(32+64+128)+(32+64+128) = 193,040 floats
        193040*4 = 772,160 bytes = 0.73638916 megabytes
*/
...
void TBVGG3_3x3ConvB(const uint depth, const uint wh, const float input[depth][wh][wh], const float error[depth][wh][wh], const uint y, const uint x, float filter[depth][9])
{
    // backprop version
    sint nx = 0, ny = 0;
    for(uint i = 0; i < depth; i++)
    {
        ...
        nx = x-1, ny = y-1;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][0] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);
            
        nx = x,   ny = y-1;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][1] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);

        nx = x+1, ny = y-1;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][2] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);

        // middle row
        nx = x-1, ny = y;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][3] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);

        nx = x,   ny = y;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][4] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);

        nx = x+1, ny = y;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][5] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);

        // top row
        nx = x-1, ny = y+1;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][6] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);

        nx = x,   ny = y+1;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][7] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);

        nx = x+1, ny = y+1;
        if(TBVGG3_CheckPadded(nx, ny, wh) == 0)
            filter[i][8] += TBVGG3_SGD(input[i][ny][nx], error[i][y][x]);
    }
}
...
float TBVGG3_Process(TBVGG3_Network* net, const float input[3][28][28], const TBVGG3_LEARNTYPE learn)
{
    ...
    else
    {
        ...
        for(uint i = 0; i < 32; i++) // num filter
        {
            net->l1fb[i][0] += TBVGG3_SGD(1, net->l1fbe[i][0]);

            for(uint j = 0; j < 28; j++) // height
                for(uint k = 0; k < 28; k++) // width
                    TBVGG3_3x3ConvB(3, 28, input, net->e1, j, k, net->l1f[i]);
        }

        // convolve filter 2 with layer 2 error gradients
        for(uint i = 0; i < 64; i++) // num filter
        {
            net->l2fb[i][0] += TBVGG3_SGD(1, net->l2fbe[i][0]);

            for(uint j = 0; j < 14; j++) // height
                for(uint k = 0; k < 14; k++) // width
                    TBVGG3_3x3ConvB(32, 14, net->o1, net->e2, j, k, net->l2f[i]);
        }

        // convolve filter 3 with layer 3 error gradients
        for(uint i = 0; i < 128; i++) // num filter
        {
            net->l3fb[i][0] += TBVGG3_SGD(1, net->l3fbe[i][0]);

            for(uint j = 0; j < 7; j++) // height
                for(uint k = 0; k < 7; k++) // width
                    TBVGG3_3x3ConvB(64, 7, net->o2, net->e3, j, k, net->l3f[i]);
        }
