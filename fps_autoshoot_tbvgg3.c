...
void processScanArea(Window w)
{
    ...
    for(uint i = 0; i < 3; i++)
    {
        for(uint j = 0; j < 28; j++)
        {
            for(uint k = 0; k < 28; k++)
            {
                if(i == 0){input[i][j][k] = ((r[(j*28)+k]-rm)+1e-7) / (rmd+1e-7);}
                if(i == 1){input[i][j][k] = ((g[(j*28)+k]-gm)+1e-7) / (gmd+1e-7);}
                if(i == 2){input[i][j][k] = ((b[(j*28)+k]-bm)+1e-7) / (bmd+1e-7);}
            }
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        if(enable == 1 && getWindow() == twin)
        {
            ...
            {
                processScanArea(twin);
                if(TBVGG3_Process(&net, &input[0], NO_LEARN) > ACTIVATION_SENITIVITY)
                {
                    // fire mouse down
                    event.type = ButtonPress;
            ...
            else
            {
                ...
                {
                    processScanArea(twin);
                    TBVGG3_Process(&net, &input[0], LEARN_MAX);
                ...
                {
                    processScanArea(twin);
                    TBVGG3_Process(&net, &input[0], LEARN_MIN);
